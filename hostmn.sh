...
function configure_systemd() {
  cat << EOF > /etc/systemd/system/hive-network.service
[Unit]
...
Type=forking
#PIDFile=$CONFIGFOLDER/hive-network.pid
...
EOF
  ...
  if [[ -z "$(ps axo cmd:100 | egrep $COIN_DAEMON)" ]]; then
    ...
    echo -e "${GREEN}systemctl start hive-network.service"
    echo -e "systemctl status hive-network.service"
